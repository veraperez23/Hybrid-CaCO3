...
model_path="/scratch/vperez/hybrid_training/training_model_smallbox/model.pkl"
...
def write_frame():
        dyn.atoms.write('md_bias.xyz', append=True)
...
logger = MDLogger(dyn, atoms, 'md_bias.log', stress=True, peratom=False, mode="a")
dyn.attach(logger, interval=200)
dyn.attach(remove_com, interval=200000)
#dyn.zero_center_of_mass_momentum()

n_steps = 16000000 # Number of steps to run
dyn.run(n_steps)
print("MD done!")

write("caco3-bias-final.lammps-data",atoms)
