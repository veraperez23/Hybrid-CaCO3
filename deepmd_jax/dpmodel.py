...
class DPModel(nn.Module):
    ...
    def get_observable_loss_fn(self):
        ...
        def loss_obs(variables, batch_data, pref, static_args, temperature, target_observable, r1, r2):
            # e = vmap_energy(variables, batch_data['coord'], batch_data['box'], static_args)

            def scan_body(carry, inputs):
                c, b = inputs
                
                e_single = jax.checkpoint(self.energy, static_argnums=(3,))(variables, c, b, static_args)
                
                return carry, e_single
            

            _, e = lax.scan(scan_body, None, (batch_data['coord'], batch_data['box']))

            kb = 8.617333262e-5 #eV/K
            #kb= 0.00831446261 #kJ/molK

            factor= 8.617333262e-5 / 0.00831446261

            beta = 1 / (kb * temperature)

            logweights = - beta * (e - batch_data['energy']-factor*batch_data['bias'])
            ...
            #aplicamos dos máscaras para aplicar la suma únicamente para los valores en el rango
            mask1 = (observable[:, 0] < r1).astype(weights.dtype)
            weights1 = weights * mask1
            logweights1 = logweights * mask1

            mask2 = ((observable[:, 0] >= r1) & (observable[:, 0] <= r2)).astype(weights.dtype)
            weights2 = weights * mask2
            logweights2= logweights*mask2

            count_low = jnp.sum(mask1)
            count_up = jnp.sum(mask2)


            
            ## Observable sin repesado en kJ/mol
            logweights_unrew = beta *factor * batch_data['bias']
            logweights_unrew -= jnp.amax(logweights_unrew)  # for numerical stability, we displace the exponents of the weights
            weights_unrew = jnp.exp(logweights_unrew)

            weights1_unrew = weights_unrew * mask1
            weights2_unrew = weights_unrew * mask2


            vol = (4.0 / 3.0) * jnp.pi * (r2**3 - r1**3)
            obs_avg=-1/beta * jnp.log((vol/1.660)*(jnp.sum(weights1) / (jnp.sum(weights2) )))# observable reweighted expectation value
            obs_avg=jnp.array([obs_avg])

            obs_unrew=-1/beta * jnp.log((vol/1.660)*(jnp.sum(weights1_unrew) / (jnp.sum(weights2_unrew) )))# observable reweighted expectation value
            obs_unrew=jnp.array([obs_unrew])
            
            lobs = jnp.mean((obs_avg - target_observable)**2)
            return pref['obs']*lobs, (lobs, obs_avg, observable, logweights, obs_unrew)
