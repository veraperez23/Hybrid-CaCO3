...
def train(
    ...
    obs_r1: Union[float, list] = None,   # radios de la función del observable
    obs_r2: Union[float, list] = None,
    ...
    if hybrid:
        ...
        labels_obs = labels + ['observable', 'bias']
        labels_obs = [item for item in labels_obs if item != 'force'] #quita la fuerza de los datos del modelo híbrido, tal vez me la dan pero yo no la quiero
        ...
        for i in range(n_paths):
            ...
            #OBSERVABLE CLEANING
            observables = single_data_obs.data['observable']
            
            # Asegurar que sea 2D
            if observables.ndim == 1:
                observables = observables[:, None]
                
            # Máscara que determina verdadero/falso
            mask_valid = (observables[:, 0] <= obs_r2)

            for clave in single_data_obs.data:
                single_data_obs.data[clave] = single_data_obs.data[clave][mask_valid]
            

            single_data_obs.nframes = int(np.sum(mask_valid))
            print(f"# Datos Observable {i}: Filtrado aplicado. Quedan {single_data_obs.nframes} frames válidos.")
            
            train_data_obs.append(single_data_obs)
        # Validate and normalize obs_r1
        if obs_r1 is None or obs_r2 is None:
            raise ValueError('Must provide obs_r1 and obs_r2 for hybrid models.')
        # Check consistency of lengths
        if isinstance(obs_r1, (int, float)):
            obs_r1 = [obs_r1] * len(obs_temperature)
        if isinstance(obs_r2, (int, float)):
            obs_r2 = [obs_r2] * len(obs_temperature)
    ...
    if init_model is None:
        ...
        print(f'# Cargando pesos anteriores desde el modelo {init_model}')
        _, variables = load_model(init_model, replicate=False)
    print('# Model initialized with parameter count %d.' %
           sum(i.size for i in jax.tree_util.tree_flatten(variables)[0]))
    
    # initialize optimizer
    if lr is None:
        lr = 0.0001 if 'atomic' not in model_type else 0.01
    ...
    if hybrid:
        _single_state_obs = {'lobs_avg': 0., 'obs_term_avg': 0., 'obs_mean': 0., 'logweights': [0.], 'ESS': 1., 'obs_unrew': 0. }
    ...
    def train_step_obs(batch, variables, opt_state, state_obs, static_args, obs_position=0):
        ...
        (loss_obs, (loss_obs_raw, obs_avg, obs_batch, logweights, obs_unrew)), grads = loss_and_grad_obs(variables,
                                                                                            batch,
                                                                                            pref,
                                                                                            static_args,
        ...
        state_obs[obs_position]['obs_unrew'] = obs_unrew
    ...
    def print_step():
        ...
        if hybrid:
            for obs_position in range(len(obs_train_data_path)):
                LOBS_train=state_obs[obs_position]["lobs_avg"]/beta_smoothing
                line += f' LOBS{obs_position} {float(LOBS_train):7.5f}'
                ...
                for obs_item  in range(len(state_obs[obs_position]["obs_term_avg"])):
                    ...
                    line += f' OBS_UNREW_{obs_position}_{obs_item} {float(state_obs[obs_position]["obs_unrew"][obs_item]):7.5f}'
